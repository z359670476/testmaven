...
System.out.print(map.get("busiCode")[0]);

//入金提交时，存储入金流水
if("10009".equals(map.get("busiCode")[0])){
	String addSql="";
	String[] param=null;
	Mazeo maz=new Mazeo();
	addSql="insert into TBL_BDPP_TRANS_FLOW (ID, MARKET_CD, TRANS_TP,SUB_TRANS_TP,SERIAL_NO,TRANS_TIME,TRANS_AT,TRANS_CURR_CD,CHANNEL_TP, IS_DEL, STATUS, CREATE_TIME,TRANS_DT,SYS_ACCT_NO,CK_ACCT) values (SEQ_BDPP_TRANS_FLOW.NEXTVAL,?,?,?,?,?,?,?,?,0,?,sysdate,TO_CHAR(sysdate, 'yyyyMMdd'),?,?) ";
	param= new String[]{map.get("marketId")[0],map.get("busiCode")[0],"01",map.get("serialNo")[0],
			map.get("transTime")[0],map.get("transAmt")[0],"156",map.get("channelType")[0],"0",map.get("acctNo")[0],map.get("acctNo")[0]};
...
if(map.containsKey("save")){
	...
	if("10001".equals(map.get("busiCode")[0])){
		...
		addSql="insert into TBL_BDPP_TRANS_FLOW (ID, MARKET_CD, TRANS_TP,SUB_TRANS_TP,SERIAL_NO,TRANS_DT,TRANS_TIME,TRANS_AT,TRANS_CURR_CD,SYS_ACCT_NO,CHANNEL_TP, IS_DEL, STATUS, CREATE_TIME) values (SEQ_BDPP_TRANS_FLOW.NEXTVAL,?,?,?,?,?,?,?,?,?,?,0,?,sysdate) ";
		param= new String[]{map.get("marketId")[0],map.get("busiCode")[0],"01",map.get("serialNo")[0],map.get("transTime")[0].substring(0, 8) ,map.get("transTime")[0],
